    ...
    m_isGrabbing(false)
{
    ...
    paramVal = ito::Param("frame_time", ito::ParamBase::Double | ito::ParamBase::Readonly, 0.0037, 0.0037, 0.0037, tr("Shortes time between two frames of CCD in s").toLatin1().data());
    m_params.insert(paramVal.getName(), paramVal);
    ...
    paramVal = ito::Param("bpp", ito::ParamBase::Int, 64, 64, 64, tr("bit depth").toLatin1().data());
    m_params.insert(paramVal.getName(), paramVal);

    paramVal = ito::Param("average", ito::ParamBase::Int, 1, 10, 1, tr("Number of averages for every frame").toLatin1().data());
    m_params.insert(paramVal.getName(), paramVal);
   ...
   /*
    DockWidgetAvantesAvaSpec *cmuwdg = new DockWidgetAvantesAvaSpec(m_params, m_uniqueID);
    connect(this, SIGNAL(parametersChanged(QMap<QString, ito::Param>)), cmuwdg, SLOT(valuesChanged(QMap<QString, ito::Param>)));
    connect(cmuwdg, SIGNAL(changeParameters(QMap<QString, ito::tParam>)), this, SLOT(updateParameters(QMap<QString, ito::tParam>)));

    Qt::DockWidgetAreas areas = Qt::AllDockWidgetAreas;
    QDockWidget::DockWidgetFeatures features = QDockWidget::DockWidgetClosable | QDockWidget::DockWidgetFloatable | QDockWidget::DockWidgetMovable;
    createDockWidget("AvantesAvaSpec", features, areas, cmuwdg);
    */
    ...
    else
    {
        ...
        if (!retValue.containsError())
        {
	        ...
	        if (average <= 1)
            {
                ...
                ito::float64 *vals = (ito::float64*)m_data.rowPtr(0,0);
        ...
        if (externalDataObject)
		{
			switch (m_params["bpp"].getVal<int>())
            {
                ...
                case 12:
                    ...
                    break;
                case 64:
                    retVal += externalDataObject->copyFromData2D<ito::float64>((ito::float64*)m_data.rowPtr(0,0), m_params["sizex"].getVal<int>(), m_params["sizey"].getVal<int>());
                    break;
                default:
