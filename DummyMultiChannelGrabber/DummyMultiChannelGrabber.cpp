...
/*!
    ...
    \brief Small interface class for class DummyMultiChannelGrabber. This class contains basic information about
   DummyMultiChannelGrabber as is able to create one or more new instances of DummyMultiChannelGrabber.
*/
...
/*!
    defines the plugin type (dataIO and grabber) and sets the plugins object name. If the real plugin (here:
   DummyMultiChannelGrabber) should or must be initialized (e.g. by a Python call) with mandatory or optional
   parameters, please initialize both vectors m_initParamsMand and m_initParamsOpt within this constructor.
*/
DummyMultiChannelGrabberInterface::DummyMultiChannelGrabberInterface()
{
    ...
    \n\
    The camera is initialized with a maximum width and height of the simulated camera chip (both need to be a multiple
    of 4). \
    The noise is always scaled in the range between 0 and the current bitdepth (bpp - bit per pixel). The real size of
    the camera \
    image is controlled using the parameter 'roi' if the sizes stay within the limits given by the size of the camera
    chip.\n\
    \n\
    You can initialize this camera either as a 2D sensor with a width and height >= 4 or as line camera whose height is
    equal to 1. \n\
    \n\
    ...
    m_detaildescription =
        QObject::tr("The DummyMultiChannelGrabber is a virtual camera which emulates a camera with white noise. \n\
\n\
...
This plugin can also be used as template for other grabber.");
    ...
    ito::Param param("maxXSize", ito::ParamBase::Int, 640, new ito::IntMeta(4, 4096, 4),
                     tr("Width of virtual sensor chip").toLatin1().data());
    m_initParamsOpt.append(param);

    param = ito::Param("maxYSize", ito::ParamBase::Int, 480, new ito::IntMeta(1, 4096, 1),
    ...
    param = ito::Param("pixelFormat", ito::ParamBase::String, "mono8",
                       tr("Bits per Pixel, usually mono8, mono10, mono12, mono16 or rgb32").toLatin1().data());
    ...
    param = ito::Param("numberOfChannels", ito::ParamBase::Int, 2, new ito::IntMeta(2, 30, 1),
                       tr("Number of channels").toLatin1().data());
    m_initParamsOpt.append(param);
}
...
//----------------------------------------------------------------------------------------------------------------------------------
// this makro registers the class DummyMultiChannelGrabberInterface with the name DummyMultiChannelGrabberinterface as
...
/*!
    ...
    \brief Class for the DummyMultiChannelGrabber. The DummyMultiChannelGrabber is able to create noisy images or
   simulate a typical WLI or confocal image signal.

    Usually every method in this class can be executed in an own thread. Only the constructor, destructor,
   showConfDialog will be executed by the main (GUI) thread.
*/

//----------------------------------------------------------------------------------------------------------------------------------
//! shows the configuration dialog. This method must be executed in the main (GUI) thread and is usually called by the
//! addIn-Manager.
/*!
    creates new instance of dialogDummyMultiChannelGrabber, calls the method setVals of dialogDummyMultiChannelGrabber,
   starts the execution loop and if the dialog is closed, reads the new parameter set and deletes the dialog.
...
/*!
    ...
    Additionally the optional docking widget for the DummyMultiChannelGrabber's toolbar is instantiated and created by
   createDockWidget.
...
/*!
    This init method gets the mandatory and optional parameter vectors of type tParam and must copy these given
   parameters to the internal m_params-vector. Notice that this method is called after that this instance has been moved
   to its own (non-gui) thread.
    ...
    \return retOk
*/
ito::RetVal DummyMultiChannelGrabber::init(QVector<ito::ParamBase> * /*paramsMand*/, QVector<ito::ParamBase> *paramsOpt,
                                           ItomSharedSemaphore *waitCond)
{
    ...
    int numChannel = paramsOpt->at(3).getVal<int>(); // third optional parameter, corresponding to the number of
                                                     // channels
    ...
    else
    {
        ...
        ito::RectMeta *rm = new ito::RectMeta(ito::RangeMeta(roi[0], roi[0] + roi[2] - 1),
                                              ito::RangeMeta(roi[1], roi[1] + roi[3] - 1), "ImageFormatControl");
        paramVal.setMeta(rm, true);
        standardParam.insert(paramVal.getName(), paramVal);

        paramVal = ito::Param("sizex", ito::ParamBase::Int | ito::ParamBase::Readonly, 4, 4096, 4096,
                              tr("size in x (cols) [px]").toLatin1().data());
        paramVal.getMetaT<ito::IntMeta>()->setCategory("ImageFormatControl");
        standardParam.insert(paramVal.getName(), paramVal);

        paramVal = ito::Param("sizey", ito::ParamBase::Int | ito::ParamBase::Readonly, 1, 4096, 4096,
        ...
        m->addItem("mono12");
        m->addItem("mono16");
        paramVal = ito::Param("pixelFormat", ito::ParamBase::String, paramsOpt->at(2).getVal<char *>(),
        ...
        channelMap["channel_1"].m_channelParam.insert(
            "channelSpecificParameter", ito::Param("channelSpecificParameter", ito::ParamBase::Int, 0, 1, 1,
                                                   tr("this is a channel specific parameter").toLatin1().data()));
        channelMap["channel_1"].m_channelParam["channelSpecificParameter"].getMetaT<ito::IntMeta>()->setCategory(
            "DemoParameters");

        // global params
        QMap<QString, ito::Param> globalParam;
        globalParam.insert("globalParam", ito::Param("globalParam", ito::ParamBase::Int, 0, 1, 1,
                                                     tr("this is a global parameter").toLatin1().data()));

        ito::DoubleMeta *dm;
        paramVal = ito::Param(
            "frame_time", ito::ParamBase::Double, 0.0, 60.0, 0.0,
            tr("Minimum time between the start of two consecutive acquisitions [s], default: 0.0.").toLatin1().data());
        ...
        paramVal = ito::Param("demoRegexpString", ito::ParamBase::String, "",
        ...
        paramVal = ito::Param("demoWildcardString", ito::ParamBase::String, "test.bmp",
...
*/
ito::RetVal DummyMultiChannelGrabber::getParameter(QSharedPointer<ito::Param> val, const ParamMapIterator &it,
    ...
    if (key == "binning")
    {
        ...
        if (!retValue.containsError())
        {
            ...
            else if ((newX != 1 && newX != 2 && newX != 4) || (newY != 1 && newY != 2 && newY != 4))
            {
                retValue += ito::RetVal(
                    ito::retError, 0,
                    "horizontal and vertical binning must be 1, 2 or 4 (hence vertical * 100 + horizontal)");
            }
            else
            {
                ...
                if (oldval != ival)
                {
                    ...
                    for (i = m_channels.begin(); i != m_channels.end(); ++i)
                    {
                        ...
                        maxWidth = static_cast<ito::RectMeta *>(i.value().m_channelParam["roi"].getMeta())
                                       ->getWidthRangeMeta()
                                       .getSizeMax();
                        maxHeight = static_cast<ito::RectMeta *>(i.value().m_channelParam["roi"].getMeta())
                                        ->getHeightRangeMeta()
                                        .getSizeMax();
...
/*!
    In the DummyMultiChannelGrabber, this method does nothing. In general, the hardware camera should be intialized in
   this method and necessary memory should be allocated.
...
/*!
    In this DummyMultiChannelGrabber, this method does nothing. In general, the hardware camera should be closed in this
   method.
...
ito::RetVal DummyMultiChannelGrabber::stopDevice(ItomSharedSemaphore *waitCond)
{
    ...
    if (grabberStartedCount() < 0)
    {
        retValue += ito::RetVal(
            ito::retWarning, 1001,
            tr("stopDevice of DummyMultiChannelGrabber can not be executed, since camera has not been started.")
                .toLatin1()
                .data());
        setGrabberStarted(0);
    }
...
/*!
    ...
    \param [in,out] vpdObj is the pointer to a given dataObject (this pointer should be cast to ito::DataObject*) where
   the acquired image is shallow-copied to. \param [in] waitCond is the semaphore (default: NULL), which is released if
   this method has been terminated \return retOk if everything is ok, retError is camera has not been started or no
   image has been acquired by the method acquire. \sa DataObject, acquire
*/
ito::RetVal DummyMultiChannelGrabber::getVal(void *vpdObj, ItomSharedSemaphore *waitCond)
{
    ...
    if (!retValue.containsError())
    {
        ...
        {
            retValue +=
                ito::RetVal(ito::retError, 1004, tr("data object of getVal is NULL or cast failed").toLatin1().data());
        }
        else
        {
            retValue += sendDataToListeners(
...
/*!
    This method copies the recently grabbed camera frame to the given DataObject. Therefore this camera size must fit to
   the data structure of the DataObject.

    \note This method is similar to VideoCapture::retrieve() of openCV

    \param [in,out] vpdObj is the pointer to a given dataObject (this pointer should be cast to ito::DataObject*) where
   the acquired image is deep copied to. \param [in] waitCond is the semaphore (default: NULL), which is released if
   this method has been terminated \return retOk if everything is ok, retError is camera has not been started or no
   image has been acquired by the method acquire. \sa DataObject, acquire
*/
...
void DummyMultiChannelGrabber::dockWidgetVisibilityChanged(bool visible)
{
    if (getDockWidget())
    {
        ...
        if (visible)
        {
            connect(this, SIGNAL(parametersChanged(QMap<QString, ito::Param>)), widget,
                    SLOT(parametersChanged(QMap<QString, ito::Param>)));
            emit parametersChanged(m_params);
        }
        else
        {
            disconnect(this, SIGNAL(parametersChanged(QMap<QString, ito::Param>)), widget,
                       SLOT(parametersChanged(QMap<QString, ito::Param>)));
        }
    }
}
