...
/*!
    ...
    \sa ito::tParam, ItomSharedSemaphore
*/
ito::RetVal DummyMultiChannelGrabber::setParam(QSharedPointer<ito::ParamBase> val, ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retValue;
    QString key;
    ...
    if (!retValue.containsError())
    {
        if (key == "defaultChannel")
        {
            ...
            if (m_data.find(it->getVal<char*>()) != m_data.end())
            {
                m_params["defaultChannel"].setVal<char*>(it->getVal<char*>());
                syncMultiChannelParams();
            }
            else
            {
    ...
    else
    {
        ...
        if (m_totalBinning == 1)
        {
            if (bpp < 9)
            {
                ...
                ito::uint8 *linePtr;
                char* a = m_params["defaultChannel"].getVal<char*>();
                ito::DataObject &channelObj = m_data[m_params["defaultChannel"].getVal<char*>()].data;
...
ito::RetVal DummyMultiChannelGrabber::getVal(void *vpdObj, ItomSharedSemaphore *waitCond)
{
    ...
    if (!retValue.containsError())
    {
        ...
        else
        {
            ...
            (*dObj) = this->m_data[m_params["defaultChannel"].getVal<char*>()].data;
        }
    }
...
ito::RetVal DummyMultiChannelGrabber::retrieveData(ito::DataObject *externalDataObject)
{
    ...
    else
    {
        ...
        {
            m_data[m_params["defaultChannel"].getVal<char*>()].data.deepCopyPartial(*externalDataObject);
        }
