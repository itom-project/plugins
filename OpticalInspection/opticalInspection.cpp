...
//----------------------------------------------------------------------------------------------------------------------------------
/*!
    \class OpticalInspectionInterface
    \brief Small interface class for class OpticalInspection. This class contains basic information about OpticalInspection as is able to
        create one or more new instances of OpticalInspection.
*/

//----------------------------------------------------------------------------------------------------------------------------------
//! creates new instance of OpticalInspection and returns the instance-pointer.
/*!
    \param [in,out] addInInst is a double pointer of type ito::AddInBase. The newly created OpticalInspection-instance is stored in *addInInst
    \return retOk
    \sa OpticalInspection
*/
ito::RetVal OpticalInspectionInterface::getAddInInst(ito::AddInBase **addInInst)
{
    NEW_PLUGININSTANCE(OpticalInspection)
    return ito::retOk;
}

//----------------------------------------------------------------------------------------------------------------------------------
//! deletes instance of OpticalInspection. This instance is given by parameter addInInst.
/*!
    \param [in] double pointer to the instance which should be deleted.
    \return retOk
    \sa OpticalInspection
*/
ito::RetVal OpticalInspectionInterface::closeThisInst(ito::AddInBase **addInInst)
{
    REMOVE_PLUGININSTANCE(OpticalInspection)
    return ito::retOk;
}
...
/*!
    defines the plugin type (dataIO and grabber) and sets the plugins object name. If the real plugin (here: OpticalInspection) should or must
...
*/
OpticalInspectionInterface::OpticalInspectionInterface()
{
    m_autoLoadPolicy = ito::autoLoadKeywordDefined;
    m_autoSavePolicy = ito::autoSaveAlways;

    m_type = ito::typeDataIO | ito::typeGrabber;
    setObjectName("OpticalInspection");

    //for the docstring, please don't set any spaces at the beginning of the line.
/*    char docstring[] = \
"The OpticalInspection is a plugin application, which combines an **ITOM** camera grabber plugin and actuator
plugin for a generalized optical inspection application \n\
*/

    m_description = QObject::tr("A virtual white noise grabber");
//    m_detaildescription = QObject::tr(docstring);
    m_detaildescription = QObject::tr(
"The OpticalInspection is a generalized application virtual camera which emulates a camera with white noise. \n\
\n\
...
This plugin can also be used as template for other grabber.");
    ...
    ito::Param param("camera", ito::ParamBase::HWRef, NULL, tr("An opened camera grabber.").toLatin1().data());
    m_initParamsMand.append(param);

    param = ito::Param("stage", ito::ParamBase::HWRef, NULL, tr("An opened stage controller.").toLatin1().data());
    m_initParamsMand.append(param);

    param = ito::Param("maxXSize", ito::ParamBase::Int, 640, new ito::IntMeta(4, 4096, 4), tr("Width of virtual sensor chip").toLatin1().data());
...
//----------------------------------------------------------------------------------------------------------------------------------
// this macro registers the class OpticalInspectionInterface with the name OpticalInspectioninterface as plugin for the Qt-System (see Qt-DOC)
...
/*!
    ...
    \brief Class for the OpticalInspection. The OpticalInspection is able to create noisy images or simulate a typical WLI or confocal image signal.
...
/*!
    creates new instance of dialogOpticalInspection, calls the method setVals of dialogOpticalInspection, starts the execution loop and if the dialog
    is closed, reads the new parameter set and deletes the dialog.

    \return retOk
    \sa dialogOpticalInspection
*/
const ito::RetVal OpticalInspection::showConfDialog(void)
{
    return apiShowConfigurationDialog(this, new DialogOpticalInspection(this));
}

//----------------------------------------------------------------------------------------------------------------------------------
//! constructor for OpticalInspection
/*!
    In this constructor the m_params-vector with all parameters, which are accessible by getParam or setParam, is built.
    Additionally the optional docking widget for the OpticalInspection's toolbar is instantiated and created by createDockWidget.

    \param [in] uniqueID is an unique identifier for this OpticalInspection-instance
    ...
    m_imageType(imgTypeNoise)
{
    ...
    ito::Param paramVal("name", ito::ParamBase::String | ito::ParamBase::Readonly, "OpticalInspection", "GrabberName");
    ...
    if (hasGuiSupport())
    {
        ...
        dockWidgetOpticalInspection *dw = new dockWidgetOpticalInspection(this);
...
//----------------------------------------------------------------------------------------------------------------------------------
//! init method which is called by the addInManager after the initiation of a new instance of OpticalInspection.
/*!
    ...
    \return retOk
*/
ito::RetVal OpticalInspection::init(QVector<ito::ParamBase> * paramsMand, QVector<ito::ParamBase> *paramsOpt, ItomSharedSemaphore *waitCond)
{
    ...
    {
        m_pCam = (ito::AddInPublicGrabber *)(*paramsMand)[0].getVal<void *>();
        ito::DataObject* m_data = m_pCam->getData();
    }
    if (reinterpret_cast<ito::AddInActuator *>((*paramsMand)[1].getVal<void *>())->getBasePlugin()->getType() & (ito::typeActuator))
    {
        m_pStage = (ito::AddInActuator *)(*paramsMand)[1].getVal<void *>();
    }
    ...
    {
        //auto val = QSharedPointer<ito::Param>::create("bpp");
        //m_pCam->getParam(val);
        //m_params["bpp"] = *val;
        //m_pCam->getParam({&m_params["bpp"], [](ito::Param*){}});
...
//----------------------------------------------------------------------------------------------------------------------------------
//! close method which is called before that this instance is deleted by the OpticalInspectionInterface
/*!
...
*/
ito::RetVal OpticalInspection::close(ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
...
*/
ito::RetVal OpticalInspection::getParam(QSharedPointer<ito::Param> val, ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retValue;
    QString key;
...
/*!
    ...
    \sa ito::tParam, ItomSharedSemaphore
*/
ito::RetVal OpticalInspection::setParam(QSharedPointer<ito::ParamBase> val, ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retValue;
    QString key;
...
/*!
    In the OpticalInspection, this method does nothing. In general, the hardware camera should be initialized in this method and necessary memory should be allocated.
...
*/
ito::RetVal OpticalInspection::startDevice(ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
...
/*!
    In this OpticalInspection, this method does nothing. In general, the hardware camera should be closed in this method.
    ...
    \sa startDevice
*/
ito::RetVal OpticalInspection::stopDevice(ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retValue = ito::retOk;
    ...
    if (grabberStartedCount() < 0)
    {
        retValue += ito::RetVal(ito::retWarning, 1001, tr("stopDevice of OpticalInspection can not be executed, since camera has not been started.").toLatin1().data());
        setGrabberStarted(0);
    }
    */
...
/*!
    ...
    \sa getVal
*/
ito::RetVal OpticalInspection::acquire(const int /*trigger*/, ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retValue = ito::retOk;
    ...
    {
        retValue += ito::RetVal(ito::retError, 1002, tr("Acquire of OpticalInspection can not be executed, since camera has not been started.").toLatin1().data());
    }
    else
    {
        m_isgrabbing = true;
...
*/
ito::RetVal OpticalInspection::getVal(void *vpdObj, ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
...
/*!
    ...
    \sa DataObject, acquire
*/
ito::RetVal OpticalInspection::copyVal(void *vpdObj, ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retValue(ito::retOk);
    ito::DataObject *dObj = reinterpret_cast<ito::DataObject *>(vpdObj);
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal OpticalInspection::retrieveData(ito::DataObject *externalDataObject)
{
    ito::RetVal retValue(ito::retOk);
...
//----------------------------------------------------------------------------------------------------------------------------------
void OpticalInspection::dockWidgetVisibilityChanged(bool visible)
{
    if (getDockWidget())
    {
