	...
	m_revision(0)
{
	...
	paramVal = ito::Param("analog_low_output_channel", ito::ParamBase::Int, 0, 7, 0, tr("first analog output channel (See pin descirption of your device).").toLatin1().data());
	m_params.insert(paramVal.getName(), paramVal);

	paramVal = ito::Param("analog_high_output_channel", ito::ParamBase::Int, 0, 7, 7, tr("last analog output channel (See pin descirption of your device).").toLatin1().data());
	m_params.insert(paramVal.getName(), paramVal);
		...
		if (voltageOutput && (inputDataType == ito::tFloat32))
		{
			...
			for (int channel = 0; channel < numChannels; channel++)
			{
				for (int sample = 0; sample < numSamples; sample++)
				{
					...
					{
						retValue += getErrStr(ret, "Error during datavalue convertion");
					}
