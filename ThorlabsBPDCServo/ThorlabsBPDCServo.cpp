...
QList<QByteArray> ThorlabsBPDCServo::openedDevices = QList<QByteArray>();


//----------------------------------------------------------------------------------------------------------------------------------
/*!
    \detail creates new instance of ThorlabsBPDCServoInterface and returns the instance-pointer
    ...
    \sa ThorlabsBP
*/
ito::RetVal ThorlabsBPDCServoInterface::getAddInInst(ito::AddInBase** addInInst)
{
    ito::RetVal retValue;
    NEW_PLUGININSTANCE(ThorlabsBPDCServo)
    return retValue;
}
...
/*!
    ...
    \sa ThorlabsBP
*/
ito::RetVal ThorlabsBPDCServoInterface::closeThisInst(ito::AddInBase** addInInst)
{

    ito::RetVal retValue;
    REMOVE_PLUGININSTANCE(ThorlabsBPDCServo)
    return retValue;
}
...
*/
ThorlabsBPDCServoInterface::ThorlabsBPDCServoInterface()
{
    m_type = ito::typeActuator;

    setObjectName("ThorlabsBPDCServo");

    m_description = QObject::tr("ThorlabsBPDCServo");
    m_detaildescription = QObject::tr("ThorlabsBPDCServo is an acutator plugin to control the following integrated devices from Thorlabs: \n\
\n\
...
//----------------------------------------------------------------------------------------------------------------------------------
// this makro registers the class ThorlabsBPDCServoInterface with the name ThorlabsBPDCServoInterface as plugin for the Qt-System (see Qt-DOC)


//----------------------------------------------------------------------------------------------------------------------------------
/*! \detail defines the name and sets the plugins parameters (m_parans). The plugin is initialized (e.g. by a Python call) 
    with mandatory or optional parameters (m_initParamsMand and m_initParamsOpt) by the ThorlabsBPDCServo::init. The widged window is created at this position.
*/
ThorlabsBPDCServo::ThorlabsBPDCServo() :
    AddInActuator(),
    m_async(0),
    m_opened(false),
    m_numChannels(0)
{
    m_params.insert("name", ito::Param("name", ito::ParamBase::String | ito::ParamBase::Readonly, "ThorlabsBPDCServo", tr("name of plugin").toLatin1().data()));
    ...
    m_params.insert("homed", ito::Param("homed", ito::ParamBase::IntArray | ito::ParamBase::Readonly, NULL, tr("If 0, the axis is not homed. 1: homed.").toLatin1().data()));
    ...
    if (hasGuiSupport())
    {
        ...
        DockWidgetThorlabsBPDCServo* dockWidget = new DockWidgetThorlabsBPDCServo(this);
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ThorlabsBPDCServo::init(
    ...
    ItomSharedSemaphore* waitCond)
{
    ...
    if (!retval.containsError())
    {
        ...
        {
            retval += ito::RetVal(ito::retError, 0, "the type of the device is not among the supported devices (Benchtop DC Servo)");
        }
    }
    ...
    if (!retval.containsError())
    {
        foreach (int channel, m_channelIndices)
        {
            ...
            int err = BDC_LoadNamedSettings(m_serialNo, channel, "M30XY");
            if (!err)
            {
        ...
        m_params["homed"].setMeta(new ito::IntArrayMeta(0, 1, 1, m_numChannels, m_numChannels, 1), true);
...
//---------------------------------------------------------------------------------------------------------------------------------
const ito::RetVal ThorlabsBPDCServo::showConfDialog(void)
{
    return apiShowConfigurationDialog(this, new DialogThorlabsBPDCServo(this));
}

//----------------------------------------------------------------------------------------------------------------------------------
/*! \detail close method which is called before that this instance is deleted by the ThorlabsBPDCServoInterface
...
*/
ito::RetVal ThorlabsBPDCServo::close(ItomSharedSemaphore* waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retval = ito::retOk;
...
/*!
    ...
    \return retOk
*/
ito::RetVal ThorlabsBPDCServo::getParam(
...
/*!
    \detail It is used to set the parameter of type char* with key "name" stored in m_params and the corresponding member variabels. 
            This function is defined by the actuator class and overwritten at this position.
            If the "ctrl-type" is set, ThorlabsBPDCServo::SMCSwitchType is executed.
    ...
    \return retOk
*/
ito::RetVal ThorlabsBPDCServo::setParam(
    ...
    if (!retValue.containsError())
    {
        ...
        else if (key == "enabled")
        {
            if (hasIndex)
            {
                if (val->getVal<int>())
                {
                    retValue += checkError(BDC_EnableChannel(m_serialNo, m_channelIndices[index]), "enable channel");
                }
                else
                {
                    retValue += checkError(BDC_DisableChannel(m_serialNo, m_channelIndices[index]), "disable channel");
                }
            ...
            else
            {
                ...
                for (int i = 0; i < m_numChannels; ++i)
                {
                    if (values[i])
                    {
                        retValue += checkError(BDC_EnableChannel(m_serialNo, m_channelIndices[i]), "enable channel");
                    }
                    else
                    {
                        retValue += checkError(BDC_DisableChannel(m_serialNo, m_channelIndices[i]), "disable channel");
                    }
                }
...
/*! \detail This function executes a calibration routine for one axis spezified by "axis". In the case of this device the function body is nearly empty and has no effect.
    ...
    \sa ItomSharedSemaphore
*/
ito::RetVal ThorlabsBPDCServo::calib(const int axis, ItomSharedSemaphore* waitCond)
{
...
/*! \detail This function executes a calibration routine for a set of axis spezified by "axis". In the case of this device the function body is nearly empty and has no effect.
    ...
    \sa ItomSharedSemaphore
*/
ito::RetVal ThorlabsBPDCServo::calib(const QVector<int> axis, ItomSharedSemaphore* waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retval;
    bool done = false;
    ...
    if (!retval.containsError())
    {
        foreach(int ax, axis)
        {
            retval += checkError(BDC_Home(m_serialNo, m_channelIndices[ax]), "set zero");
        }
...
//----------------------------------------------------------------------------------------------------------------
ito::RetVal ThorlabsBPDCServo::setOrigin(const int axis, ItomSharedSemaphore* waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
...
//----------------------------------------------------------------------------------------------------------------
ito::RetVal ThorlabsBPDCServo::setOrigin(QVector<int> axis, ItomSharedSemaphore* waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retval = ito::RetVal(ito::retWarning, 0, "set origin not implemented.");
...
*/
ito::RetVal ThorlabsBPDCServo::getStatus(
    QSharedPointer<QVector<int>> status, ItomSharedSemaphore* waitCond)
{
    ...
    for (int idx = 0; idx < m_numChannels; ++idx)
    {
        ...
        if (memcmp(m_params["homed"].getVal<int*>(), zeroed, m_numChannels * sizeof(int)) != 0 || \
            memcmp(m_params["enabled"].getVal<int*>(), enabled, m_numChannels * sizeof(int)) != 0)
        {
            m_params["homed"].setVal<int*>(zeroed, m_numChannels);
...
/*! \detail Get the Position of a single axis spezified by axis. The value in device independet in mm.
    ...
    \return retOk
*/
ito::RetVal ThorlabsBPDCServo::getPos(
...
/*! \detail Set the absolute position of a one axis spezified by "axis" to the position "pos" . The value in device independet in mm. 
            This function calls ThorlabsBPDCServo::SMCSetPos(axis, pos, "ABSOLUTCOMMAND")
    ...
    \return retOk
*/
ito::RetVal ThorlabsBPDCServo::setPosAbs(
    ...
    const QVector<int> axis, QVector<double> pos, ItomSharedSemaphore* waitCond)
{
    ...
    if (!retval.containsError())
    {
        ...
        for (int i = 0; i < axis.size(); ++i)
        {
            ...
            retval += checkError(BDC_MoveToPosition(m_serialNo,m_channelIndices[idx], m_targetPos[idx]), "set position");
        }
...
/*! \detail Set the relativ position of a one axis spezified by "axis" to the position "pos" . The value in device independet in mm. 
    ...
    \return retOk
*/
ito::RetVal ThorlabsBPDCServo::setPosRel(
    const int axis, const double pos, ItomSharedSemaphore* waitCond)
{
    QVector<int> axes(1, axis);
    QVector<double> positions(1, pos);
...
/*! \detail This slot is triggerd by the request signal from the dockingwidged dialog to update the position after ever positioning command.
    ...
    \return retOk
*/
ito::RetVal ThorlabsBPDCServo::requestStatusAndPosition(bool sendCurrentPos, bool sendTargetPos)
{
    ito::RetVal retval(ito::retOk);
...
//----------------------------------------------------------------------------------------------------------------
ito::RetVal ThorlabsBPDCServo::waitForDone(
    const int timeoutMS,
...
//---------------------------------------------------------------------------------------------------------------------------------- 
void ThorlabsBPDCServo::dockWidgetVisibilityChanged(bool visible)
{
    if (getDockWidget())
    {
...
//---------------------------------------------------------------------------------------------------------------------------------- 
ito::RetVal ThorlabsBPDCServo::checkError(short value, const char* message)
{
    if (value == 0)
    {
        return ito::retOk;
