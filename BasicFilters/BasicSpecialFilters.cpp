...
//----------------------------------------------------------------------------------------------------------------------------------
/*!\detail copy the columns from different z-planes of a multi-plane single column object to a
   single plane 2D-Object \param[in]   dObjSrc     DataObject with shape Z x Y x 1 \param[out]
   dObjDst     DataObject with shape Y x (Z=X) \author ITO \date
*/
...
/*!\detail
   ...
   \date 02.2012
*/
ito::RetVal BasicFilters::flaten3Dto2D(
    ...
    QVector<ito::ParamBase>* /*paramsOut*/)
{
    ...
    if (dObjSrc->getSize(1) != 1 && dObjSrc->getSize(2) != 1)
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("Error: one dimension of input image must be equal to 1").toLatin1().data());
    }
    ...
    else
    {
        if (keepX)
        {
            if (dObjSrc->getSize(0) != dObjDst->getSize(0) ||
                dObjSrc->getSize(2) != dObjDst->getSize(1))
            {
                needNewObj = true;
                xsize = dObjSrc->getSize(2);
                ysize = dObjSrc->getSize(0);
        ...
        {
            if (dObjSrc->getSize(0) != dObjDst->getSize(1) ||
                dObjSrc->getSize(1) != dObjDst->getSize(0))
            {
                needNewObj = true;
                xsize = dObjSrc->getSize(0);
                ysize = dObjSrc->getSize(1);
    ...
    case ito::tInt8:
        if (keepX)
            copyRows<ito::int8>(dObjSrc, &tempObj);
        else
            copyCols<ito::int8>(dObjSrc, &tempObj);
        break;
    case ito::tUInt8:
        if (keepX)
            copyRows<ito::uint8>(dObjSrc, &tempObj);
        else
            copyCols<ito::uint8>(dObjSrc, &tempObj);
        break;
    case ito::tInt16:
        if (keepX)
            copyRows<ito::int16>(dObjSrc, &tempObj);
        else
            copyCols<ito::int16>(dObjSrc, &tempObj);
        break;
    case ito::tUInt16:
        if (keepX)
            copyRows<ito::uint16>(dObjSrc, &tempObj);
        else
            copyCols<ito::uint16>(dObjSrc, &tempObj);
        break;
    case ito::tInt32:
        if (keepX)
            copyRows<ito::int32>(dObjSrc, &tempObj);
        else
            copyCols<ito::int32>(dObjSrc, &tempObj);
        break;
    case ito::tFloat32:
        if (keepX)
            copyRows<ito::float32>(dObjSrc, &tempObj);
        else
            copyCols<ito::float32>(dObjSrc, &tempObj);
        break;
    case ito::tFloat64:
        if (keepX)
            copyRows<ito::float32>(dObjSrc, &tempObj);
    ...
    default:
        return ito::RetVal(
            ito::retError, 0, tr("Unknown type or type not implemented").toLatin1().data());
    }
...
/*!\detail
   ...
   \date 02.2012
*/
ito::RetVal BasicFilters::swapByteOrder(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* /*paramsOpt*/,
    QVector<ito::ParamBase>* /*paramsOut*/)
{
    ...
    if ((retval += ito::dObjHelper::verifyDataObjectType(
             dObjSrc, "dObjSrc", 3, ito::tInt32, ito::tUInt16, ito::tInt16))
            .containsError())
        return retval;

    if (dObjSrc == dObjDst) // If both pointer are equal or the object are equal take it else make a
                            // new destObject
    {
        // Nothing
    }
    ...
    else
    {
        (*dObjDst) = ito::DataObject(
            dObjSrc->getDims(), dObjSrc->getSize(), dObjSrc->getType(), dObjSrc->getContinuous());
...
//-------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::replaceInfAndNaN(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* /*paramsOpt*/,
    QVector<ito::ParamBase>* paramsOut)
{
    ...
    {
        return ito::RetVal(
            ito::retError, 0, tr("Error: replace image ptr empty").toLatin1().data());
    }
    ...
    if (!ito::dObjHelper::dObjareEqualShort(dObjSrc, dObjReplace))
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("source and replace image must have the same type and size").toLatin1().data());
    }
    ...
    if (dObjSrc->getType() != ito::tFloat32 && dObjSrc->getType() != ito::tFloat64)
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("Error: this filter is only usable for float or double matrices").toLatin1().data());
    }
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::replaceInfAndNaNParams(
    QVector<ito::Param>* paramsMand, QVector<ito::Param>* paramsOpt, QVector<ito::Param>* paramsOut)
{
    ...
    if (!retval.containsError())
    {
        ito::Param param = ito::Param(
            ...
            NULL,
            tr("Input object of type float32 or float64 whose non-finite values will be replaced "
               "with the values in replaceImg.")
                .toLatin1()
                .data());
        ...
        param = ito::Param(
            ...
            ito::ParamBase::DObjPtr | ito::ParamBase::In,
            NULL,
            tr("replacement data object of same type and shape than srcImg.").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "destImg",
            ito::ParamBase::DObjPtr | ito::ParamBase::In | ito::ParamBase::Out,
            NULL,
            tr("Output object of same type and shape than srcImg (inplace possible).")
                .toLatin1()
                .data());
        paramsMand->append(param);
        ...
        paramsOut->append(ito::Param(
            "nrOfReplacements",
            ito::ParamBase::Int | ito::ParamBase::Out,
            0,
            NULL,
            tr("number of values that have been replaced.").toLatin1().data()));
    }
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::mergeColorPlanesParams(
    QVector<ito::Param>* paramsMand, QVector<ito::Param>* paramsOpt, QVector<ito::Param>* paramsOut)
{
    ito::RetVal retval = prepareParamVectors(paramsMand, paramsOpt, paramsOut);
    if (!retval.containsError())
    {
        ito::Param param = ito::Param(
            ...
            ito::ParamBase::DObjPtr | ito::ParamBase::In,
            NULL,
            tr("Input image with 3 or 4 uint8 planes").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "destImg",
            ito::ParamBase::DObjPtr | ito::ParamBase::In | ito::ParamBase::Out,
            NULL,
            tr("Output image with of type rgba32").toLatin1().data());
        paramsMand->append(param);
        ...
        param = ito::Param(
            ...
            3,
            0,
            tr("Switch between RGBA = 0, BGRA = 1, ARGB = 2, ABGR = 3").toLatin1().data());
...
//-------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::mergeColorPlane(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* paramsOpt,
    QVector<ito::ParamBase>* /*paramsOut*/)
{
    ...
    //    if (dObjSrc->getType() != ito::tUInt8 && dObjSrc->getDims() != 3 && (numMats != 3 ||
    //    numMats != 4))
    // TODO: malformed if statement changed || to && is this what was intended?
    if (dObjSrc->getType() != ito::tUInt8 || (dObjSrc->getDims() != 3) ||
        (numMats != 3 && numMats != 4))
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("SrcImg must be three dimensional, of type uint8 and contain three or four planes.")
                .toLatin1()
                .data());
    }

    int planeSize[2] = {dObjSrc->getSize(1 - isCVMat), dObjSrc->getSize(2 - isCVMat)},
        useTmpObj = 0;
    bool check;

    if ((dObjSrc == dObjDst) ||
        (dObjDst->getType() != ito::tUInt32 && dObjDst->getType() != ito::tRGBA32) ||
        dObjDst->getDims() != 2 || planeSize[0] != dObjDst->getSize(0) ||
    ...
    const cv::Mat_<ito::uint8>* matrixR = NULL;
    const cv::Mat_<ito::uint8>* matrixG = NULL;
    const cv::Mat_<ito::uint8>* matrixB = NULL;
    const cv::Mat_<ito::uint8>* matrixA = NULL;
    ...
    if (!isCVMat)
    {
        ...
        {
        case 0:
            matrixR = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(0));
            matrixG = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(1));
            matrixB = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(2));
            if (numMats == 4)
                matrixA = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(3));
            break;
        case 1:
            matrixB = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(0));
            matrixG = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(1));
            matrixR = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(2));
            if (numMats == 4)
                matrixA = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(3));
            break;
        case 2:
            matrixA = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(0));
            matrixR = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(1));
            matrixG = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(2));
            matrixB = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(3));
            break;
        case 3:
            matrixA = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(0));
            matrixB = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(1));
            matrixG = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(2));
            matrixR = (cv::Mat_<ito::uint8>*)(dObjSrc->getCvPlaneMat(3));
            break;
        }
        ...
        if (numMats == 4 && convertToInt32)
        {
            ...
            {
                rowPtrR = matrixR->ptr<ito::uint8>(y);
                rowPtrG = matrixG->ptr<ito::uint8>(y);
                rowPtrB = matrixB->ptr<ito::uint8>(y);
                rowPtrA = matrixA->ptr<ito::uint8>(y);
    ...
    else // isCVMat
    {
        if (!dObjSrc->getContinuous())
        {
            retval += ito::RetVal(
                ito::retError,
                0,
                tr("Can convert only continuous cvMat style matrices to rgb32").toLatin1().data());
        }
        else
        {
                ...
                for (int y = 0; y < planeSize[0]; y++)
                {
                    for (int x = 0; x < planeSize[1]; x++)
                    {
                        rowPtrDst[y * planeSize[1] + x] =
                            srcPtr[(y * planeSize[1] + x) * numMats + ofsR];
                        rowPtrDst[y * planeSize[1] + x] +=
                            srcPtr[(y * planeSize[1] + x) * numMats + ofsG] << 8;
                        rowPtrDst[y * planeSize[1] + x] +=
                            srcPtr[(y * planeSize[1] + x) * numMats + ofsB] << 16;
                        rowPtrDst[y * planeSize[1] + x] +=
                            srcPtr[(y * planeSize[1] + x) * numMats + ofsA] << 24;
                    }
                }
            }
            else if (convertToInt32)
            ...
            else if (numMats == 4)
            {
                ...
                for (int y = 0; y < planeSize[0]; y++)
                {
                    for (int x = 0; x < planeSize[1]; x++)
                    {
                        rowPtrDst[y * planeSize[1] + x].r =
                            srcPtr[(y * planeSize[1] + x) * numMats + ofsR];
                        rowPtrDst[y * planeSize[1] + x].g =
                            srcPtr[(y * planeSize[1] + x) * numMats + ofsG];
                        rowPtrDst[y * planeSize[1] + x].b =
                            srcPtr[(y * planeSize[1] + x) * numMats + ofsB];
                        rowPtrDst[y * planeSize[1] + x].a =
                            srcPtr[(y * planeSize[1] + x) * numMats + ofsA];
                    }
                }
            }
            else
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::calcMeanOverZParams(
    QVector<ito::Param>* paramsMand, QVector<ito::Param>* paramsOpt, QVector<ito::Param>* paramsOut)
{
    ito::RetVal retval = prepareParamVectors(paramsMand, paramsOpt, paramsOut);
    if (!retval.containsError())
    {
        ito::Param param = ito::Param(
            ...
            ito::ParamBase::DObjPtr | ito::ParamBase::In,
            NULL,
            tr("3D input object of dimension [ZxMxN] with Z different planes.").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "destImg",
            ito::ParamBase::DObjPtr | ito::ParamBase::In | ito::ParamBase::Out,
            NULL,
            tr("Output image of same type than input object and dimension [1xMxN] or [2xMxN] "
               "depending on parameter 'calcStd'.")
                .toLatin1()
                .data());
        paramsMand->append(param);
        ...
        param = ito::Param(
            ...
            1,
            1,
            tr("If 1, Inf or NaN values will not be taken into account when calculating the mean "
               "value (default, only important for floating point data types).")
                .toLatin1()
                .data());
        paramsOpt->append(param);
        ...
        param = ito::Param(
            ...
            1,
            1,
            tr("If 1, the standard deviation is also calculated and put into the second plane of "
               "'destImg'.")
                .toLatin1()
                .data());
        paramsOpt->append(param);
...
//----------------------------------------------------------------------------------------------------------------------------------
template <typename _Type, typename _TypeDst>
void calcMeanOverZHelp(_Type*** srcPtr, cv::Mat* dstMat, const int* sizes, const bool toggleInf)
{
    _TypeDst* linePtr = 0;
...
//----------------------------------------------------------------------------------------------------------------------------------
template <typename _Type, typename _TypeDst>
void calcDeviationOverZHelp(
    _Type*** srcPtr, cv::Mat* meanMat, cv::Mat* dstMat, const int* sizes, const bool toggleInf)
{
    _TypeDst* linePtr = 0;
    ...
    if (std::numeric_limits<_Type>::is_exact)
    {
        for (int y = 0; y < sizes[1]; y++)
        {
            ...
            for (int x = 0; x < sizes[2]; x++)
            {
                ...
                {
                    value +=
                        pow((ito::float64)(srcPtr[z][y][x]) - (ito::float64)(meanlinePtr[x]), 2);
                    cnts++;
                }
            ...
            for (int y = 0; y < sizes[1]; y++)
            {
                ...
                for (int x = 0; x < sizes[2]; x++)
                {
                    ...
                    for (int z = 0; z < sizes[0]; z++)
                    {
                        ...
                        {
                            value += pow(
                                (ito::float64)(srcPtr[z][y][x]) - (ito::float64)(meanlinePtr[x]),
                                2);
                            cnts++;
                        }
                    }
        ...
        else
        {
            for (int y = 0; y < sizes[1]; y++)
            {
                ...
                for (int x = 0; x < sizes[2]; x++)
                {
                    ...
                    {
                        value += pow(
                            (ito::float64)(srcPtr[z][y][x]) - (ito::float64)(meanlinePtr[x]), 2);
                        cnts++;
                    }
                    if (cnts > 0.0)
                    {
...
/*!\detail
   ...
   \date 02.2012
*/
ito::RetVal BasicFilters::calcMeanOverZ(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* paramsOpt,
    QVector<ito::ParamBase>* /*paramsOut*/)
{
    ...
    {
        return ito::RetVal(
            ito::retError, 0, tr("Error: sourceImageStack is Null-Pointer").toLatin1().data());
    }
    ...
    if (dObjDst == NULL)
    {
        retval += ito::RetVal(
            ito::retError,
            0,
            tr("destinationPlane is a uninitialized dataObject!").toLatin1().data());
    }
    ...
    case ito::tInt8: {
        ...
        calcMeanOverZHelp<ito::int8, ito::int8>(
            srcPtr, (cv::Mat*)(destPlane.get_mdata()[0]), sizes, toggleInf);
        if (nessPlanes > 1)
            calcDeviationOverZHelp<ito::int8, ito::int8>(
                srcPtr,
                (cv::Mat*)(destPlane.get_mdata()[0]),
                (cv::Mat*)(destPlane.get_mdata()[1]),
                sizes,
                toggleInf);
    ...
    case ito::tUInt8: {
        ...
        calcMeanOverZHelp<ito::uint8, ito::uint8>(
            srcPtr, (cv::Mat*)(destPlane.get_mdata()[0]), sizes, toggleInf);
        if (nessPlanes > 1)
            calcDeviationOverZHelp<ito::uint8, ito::uint8>(
                srcPtr,
                (cv::Mat*)(destPlane.get_mdata()[0]),
                (cv::Mat*)(destPlane.get_mdata()[1]),
                sizes,
    ...
    case ito::tInt16: {
        ...
        calcMeanOverZHelp<ito::int16, ito::int16>(
            srcPtr, (cv::Mat*)(destPlane.get_mdata()[0]), sizes, toggleInf);
        if (nessPlanes > 1)
            calcDeviationOverZHelp<ito::int16, ito::int16>(
                srcPtr,
                (cv::Mat*)(destPlane.get_mdata()[0]),
                (cv::Mat*)(destPlane.get_mdata()[1]),
                sizes,
    ...
    case ito::tUInt16: {
        ...
        calcMeanOverZHelp<ito::uint16, ito::uint16>(
            srcPtr, (cv::Mat*)(destPlane.get_mdata()[0]), sizes, toggleInf);
        if (nessPlanes > 1)
            calcDeviationOverZHelp<ito::uint16, ito::uint16>(
                srcPtr,
                (cv::Mat*)(destPlane.get_mdata()[0]),
                (cv::Mat*)(destPlane.get_mdata()[1]),
                sizes,
    ...
    case ito::tInt32: {
        ...
        calcMeanOverZHelp<ito::int32, ito::int32>(
            srcPtr, (cv::Mat*)(destPlane.get_mdata()[0]), sizes, toggleInf);
        if (nessPlanes > 1)
            calcDeviationOverZHelp<ito::int32, ito::int32>(
                srcPtr,
                (cv::Mat*)(destPlane.get_mdata()[0]),
                (cv::Mat*)(destPlane.get_mdata()[1]),
                sizes,
    ...
    case ito::tFloat32: {
        ...
        calcMeanOverZHelp<ito::float32, ito::float32>(
            srcPtr, (cv::Mat*)(destPlane.get_mdata()[0]), sizes, toggleInf);
        if (nessPlanes > 1)
            calcDeviationOverZHelp<ito::float32, ito::float32>(
                srcPtr,
                (cv::Mat*)(destPlane.get_mdata()[0]),
                (cv::Mat*)(destPlane.get_mdata()[1]),
                sizes,
    ...
    case ito::tFloat64: {
        ...
        calcMeanOverZHelp<ito::float64, ito::float64>(
            srcPtr, (cv::Mat*)(destPlane.get_mdata()[0]), sizes, toggleInf);
        if (nessPlanes > 1)
            calcDeviationOverZHelp<ito::float64, ito::float64>(
                srcPtr,
                (cv::Mat*)(destPlane.get_mdata()[0]),
                (cv::Mat*)(destPlane.get_mdata()[1]),
                sizes,
...
/*!\detail Filter parameters for calcObjSlice
   ...
   \date 02.2012
*/
ito::RetVal BasicFilters::calcObjSliceParams(
    QVector<ito::Param>* paramsMand, QVector<ito::Param>* paramsOpt, QVector<ito::Param>* paramsOut)
{
    ito::RetVal retval = prepareParamVectors(paramsMand, paramsOpt, paramsOut);
    if (!retval.containsError())
    {
        ito::Param param = ito::Param(
            ...
            ito::ParamBase::DObjPtr | ito::ParamBase::In,
            NULL,
            tr("2D image or single plane n-D object").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "dstSlice",
            ito::ParamBase::DObjPtr | ito::ParamBase::In | ito::ParamBase::Out,
            NULL,
            tr("Slice with output data").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "x0",
            ...
            std::numeric_limits<ito::float32>::max(),
            0.0,
            tr("x0-coordinate for slice").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "y0",
            ito::ParamBase::Double,
            ...
            std::numeric_limits<ito::float32>::max(),
            1.0,
            tr("y0-coordinate for slice").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "x1",
            ito::ParamBase::Double,
            ...
            std::numeric_limits<ito::float32>::max(),
            0.0,
            tr("x1-coordinate for slice").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "y1",
            ito::ParamBase::Double,
            ...
            std::numeric_limits<ito::float32>::max(),
            1.0,
            tr("y1-coordinate for slice").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "interpolationMode",
            ito::ParamBase::Int,
            ...
            0,
            0,
            tr("0: Bresenham or Nearest Neighbour, 1: weighted (not implemented, yet).")
                .toLatin1()
                .data());
        paramsOpt->append(param);
    }
...
/*!\detail
   ...
   \date 02.2012
*/
ito::RetVal BasicFilters::calcObjSlice(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* paramsOpt,
    QVector<ito::ParamBase>* /*paramsOut */)
{
    ...
    {
        return ito::RetVal(
            ito::retError, 0, tr("Error: sourceImage is Null-Pointer").toLatin1().data());
    }
    ...
    if (dims < 2)
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("Error: sourceImage must have at least 2 dimensions").toLatin1().data());
    }
    ...
    if (dObjSrc->calcNumMats() > 1)
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("Error: sourceImage must not have more than 1 plane").toLatin1().data());
    }
    ...
    */
    ...
    if (pxX0 == pxX1) // pure line in y-direction
    {
        ...
        if (pxY1 >= pxY0)
        {
            ...
            stepSizePhys =
                sliceXSize > 1 ? (right - startPhys) / (ito::float64)(sliceXSize - 1) : 0.0;
    ...
    else
    {
        ...
        if (true)
        {
            ...
            if (sliceXSize > 0)
            {
                double dxPhys = dObjSrc->getPixToPhys(dims - 1, pxX1, _unused) -
                    dObjSrc->getPixToPhys(dims - 1, pxX0, _unused);
                double dyPhys = dObjSrc->getPixToPhys(dims - 2, pxY1, _unused) -
                    dObjSrc->getPixToPhys(dims - 2, pxY0, _unused);
        ...
        if (dObjSrc->getAxisUnit(dims - 1, _unused) == dObjSrc->getAxisUnit(dims - 2, _unused))
            axisUnit = dObjSrc->getAxisUnit(dims - 1, _unused);
        else
            axisUnit = "";
    ...
    if (!retval.containsError())
    {
        ...
        {
            if ((dObjDst->getSize(0) != 1) || (dObjDst->getSize(1) != sliceXSize) ||
                (dObjDst->getType() != dObjSrc->getType()))
            {
                needNewObj = true;
            }
        }
    ...
    {
        retval += ito::RetVal(
            ito::retError, 0, tr("matrix step vector for matrix is empty").toLatin1().data());
    }
    ...
    if (!retval.containsError())
    {
        ...
        QString msg = tr("Cut 1D slice out of 2D-data from [ %2, %3] to [ %4, %5]")
                          .arg(physX0)
                          .arg(physY0)
                          .arg(physX1)
                          .arg(physY1);
        ...
        if (ito::isNotZero(stepSizePhys))
            dObjDst->setAxisScale(1, stepSizePhys);
        else
            dObjDst->setAxisScale(1, stepSizePhys);
...
ito::RetVal clipValuesHelper(
    cv::Mat* planeInOut, _Tp minVal, _Tp maxVal, _Tp newValue, bool outside)
{
#if (USEOMP)
...
#pragma omp for schedule(guided)
            ...
            for (y = 0; y < planeInOut->rows; y++)
            {
                ...
                for (x = 0; x < planeInOut->cols; x++)
                {
                    ...
                    if (rowPtr[x] > minVal && rowPtr[x] < maxVal)
                    {
                        rowPtr[x] = newValue;
                    }
                }
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::clipValueFilterParams(
    QVector<ito::Param>* paramsMand, QVector<ito::Param>* paramsOpt, QVector<ito::Param>* paramsOut)
{
    ito::Param param;
    ito::RetVal retval = ito::retOk;
    ...
    param = ito::Param(
        ...
        ito::ParamBase::DObjPtr | ito::ParamBase::In,
        NULL,
        tr("input image [real typed data object]").toLatin1().data());
    paramsMand->append(param);
    param = ito::Param(
        "destinationImage",
        ito::ParamBase::DObjPtr | ito::ParamBase::In | ito::ParamBase::Out,
        NULL,
        tr("destination image (inplace possible)").toLatin1().data());
    paramsMand->append(param);
    param = ito::Param(
        "minValue",
        ito::ParamBase::Double,
        ...
        std::numeric_limits<double>::max(),
        0.0,
        tr("lowest value in range").toLatin1().data());
    paramsMand->append(param);
    param = ito::Param(
        "maxValue",
        ito::ParamBase::Double,
        ...
        std::numeric_limits<double>::max(),
        0.0,
        tr("highest value in range").toLatin1().data());
    paramsMand->append(param);
    ...
    param = ito::Param(
        ...
        std::numeric_limits<double>::max(),
        0.0,
        tr("value set to clipped values (default: 0.0)").toLatin1().data());
    paramsOpt->append(param);
    param = ito::Param(
        "insideFlag",
        ito::ParamBase::Int,
        ...
        1,
        0,
        tr("0: clip values outside of given range (default), 1: clip inside").toLatin1().data());
    paramsOpt->append(param);
...
/*!\detail
   ...
   \date
*/
ito::RetVal BasicFilters::clipValueFilter(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* paramsOpt,
    QVector<ito::ParamBase>* /*paramsOut*/)
{
    ...
    if ((*paramsMand)[2].getVal<double>() > (*paramsMand)[3].getVal<double>())
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("Error: minValue must be smaller than maxValue").toLatin1().data());
    }
    ...
    default:
        return ito::RetVal(
            ito::retError,
            0,
            tr("unknown type or type not implemented (e.g. complex)").toLatin1().data());
    }
    ...
    if (!retval.containsError())
    {
        ...
        if (outsideFlag)
        {
            msg = tr("Clipped values outside %1 : %2 to %3")
                      .arg(minValProt)
                      .arg(maxValProt)
                      .arg(valProt);
        }
        ...
        {
            msg = tr("Clipped values inside %1 : %2 to %3")
                      .arg(minValProt)
                      .arg(maxValProt)
                      .arg(valProt);
        }
...
*/
template <typename _Tp>
ito::RetVal clipAbyBFirstHelper(
    const cv::Mat* planeComp, cv::Mat& planeBinary, _Tp minVal, _Tp maxVal, bool outside)
{
#if (USEOMP)
#pragma omp parallel num_threads(ito::AddInBase::getMaximumThreadCount())
    {
...
#pragma omp for schedule(guided)
            ...
            for (y = 0; y < planeBinary.rows; y++)
            {
                ...
                for (x = 0; x < planeBinary.cols; x++)
                {
                    ...
                    if (rowPtrComp[x] > minVal && rowPtrComp[x] < maxVal)
                    {
                        rowPtrInOut[x] = 0;
                    }
                    else
...
template <typename _Tp>
ito::RetVal clipAbyBSecondHelper(const cv::Mat& planeBinary, cv::Mat* planeInOut, _Tp newValue)
{
#if (USEOMP)
#pragma omp parallel num_threads(ito::AddInBase::getMaximumThreadCount())
    {
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::clipAbyBFilterParams(
    QVector<ito::Param>* paramsMand, QVector<ito::Param>* paramsOpt, QVector<ito::Param>* paramsOut)
{
    ito::Param param;
    ito::RetVal retval = ito::retOk;
    ...
    param = ito::Param(
        ...
        ito::ParamBase::DObjPtr | ito::ParamBase::In,
        NULL,
        tr("input image [real typed data object] for comparison").toLatin1().data());
    paramsMand->append(param);
    param = ito::Param(
        "destinationImage",
        ito::ParamBase::DObjPtr | ito::ParamBase::In | ito::ParamBase::Out,
...
/*!\detail
   ...
   \date
*/
ito::RetVal BasicFilters::clipAbyBFilter(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* paramsOpt,
    QVector<ito::ParamBase>* /*paramsOut*/)
{
...
*/
template <typename _Tp>
ito::RetVal HistogramBlock(
    const cv::Mat* planeIn, cv::Mat* histOut, const double& min, const double& max)
{
    ito::int32* rowPtrOut;
...
*/
template <>
ito::RetVal HistogramBlock<ito::Rgba32>(
    const cv::Mat* planeIn, cv::Mat* histOut, const double& min, const double& max)
{
...
//----------------------------------------------------------------------------------------------------------------------------------
const QString BasicFilters::calcHistDoc =
    QObject::tr("calculates histogram of real input data object.");

//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::calcHistParams(
    QVector<ito::Param>* paramsMand, QVector<ito::Param>* paramsOpt, QVector<ito::Param>* paramsOut)
{
    ito::Param param;
    ito::RetVal retval = ito::retOk;
    ...
    param = ito::Param(
        ...
        ito::ParamBase::DObjPtr | ito::ParamBase::In,
        NULL,
        tr("2D or multidimensional source data object ((u)int8, (u)int16, (u)int32, float32, "
           "float64 or rgba32)")
            .toLatin1()
            .data());
    paramsMand->append(param);
    param = ito::Param(
        ...
        ito::ParamBase::DObjPtr | ito::ParamBase::In | ito::ParamBase::Out,
        NULL,
        tr("histogram data object (will be int32, [higher-dimensions x 1 x bins] where "
           "higher-dimensions corresponds to the dimensions higher than x and y of the source "
           "object. A source object of type rgba32 will lead to [higher-dimensions x 4 x bins] "
           "where the 4-sized dimension is the histogram if the [r,g,b,gray] value channels. If "
           "the given object already fits to the type and size requirements, it is used without "
    ...
    param = ito::Param(
        ...
        4096,
        0,
        tr("Number of bins ((u)int16, (u)int32, float32 and float64 only, default: 0 leads to 1024 "
           "bins), for (u)int8 and rgba32 the number of bins is given by the total number of "
           "values represented by the data type.")
            .toLatin1()
            .data());
    paramsOpt->append(param);
    param = ito::Param(
        ...
        1,
        -1,
        tr("Defines how to determine the interval of the histogram: -1 (default) use the limits of "
           "(u)int8 and rgba32 and auto-calculate the min/max values for floating point data "
           "types, 0: use the values given by interval, 1: automatically calculate the min/max "
    ...
    param = ito::Param(
        ...
        2,
        limits,
        tr("Interval of the histogram (depending on parameter 'autoInterval'). The first value is "
           "included in the first bin, the last value is included in the last bin.")
            .toLatin1()
            .data());
    param.setMeta(
        new ito::DoubleIntervalMeta(
...
/*!\detail
   ...
   \date
*/
ito::RetVal BasicFilters::calcHistFilter(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* paramsOpt,
    QVector<ito::ParamBase>* /*paramsOut*/)
{
    ...
    default:
        return ito::RetVal(
            ito::retError,
            0,
            tr("Histogram can only be calculated for (u)int8, (u)int16, (u)int32, float32, float64 "
               "or rgba32")
                .toLatin1()
                .data());
    }
    ...
    if (dObjDestination ==
        NULL) // given destination object does not fit to requirements, allocate a new one
    {
    ...
    if (recalcMinMax)
    {
        ...
        if (fabs(maxVal - minVal) < std::numeric_limits<double>::epsilon() * hbins)
        {
            ...
            {
                minVal +=
                    (std::numeric_limits<double>::epsilon() * hbins - fabs(maxVal - minVal)) / 2.0;
                maxVal -=
                    (std::numeric_limits<double>::epsilon() * hbins - fabs(maxVal - minVal)) / 2.0;
            }
            else
    ...
    if (!retval.containsError())
    {
        ...
        for (int planes = 0; planes < dObjImages->getDims() - 2; planes++)
        {
            ...
            if (isValid)
                dObjDestination->setAxisDescription(planes, tempDes);
            isValid = false;
            std::string tempUnit = dObjImages->getAxisUnit(planes, isValid);
            if (isValid)
                dObjDestination->setAxisUnit(planes, tempUnit);
        }
        ...
        dObjDestination->setAxisDescription(
            dObjDestination->getDims() - 1, std::string((*dObjImages).getValueDescription()));
        dObjDestination->setAxisUnit(
            dObjDestination->getDims() - 1, std::string((*dObjImages).getValueUnit()));
        ;
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::fillGeometricParams(
    QVector<ito::Param>* paramsMand, QVector<ito::Param>* paramsOpt, QVector<ito::Param>* paramsOut)
{
    ito::Param param;
    ito::RetVal retval = ito::retOk;
    ...
    param = ito::Param(
        ...
        ito::ParamBase::DObjPtr | ito::ParamBase::In | ito::ParamBase::Out,
        NULL,
        tr("Preallocated dataObject to be filled").toLatin1().data());
    paramsMand->append(param);
    param = ito::Param(
        "geometricElement",
        ito::ParamBase::DObjPtr | ito::ParamBase::In,
        NULL,
        tr("Geometric primitive according to definition").toLatin1().data());
    paramsMand->append(param);
    param = ito::Param(
        "insideFlag",
        ito::ParamBase::Int,
        ...
        3,
        1,
        tr("Switch between fill inside, outside or both").toLatin1().data());
    paramsOpt->append(param);
    param = ito::Param(
        "edgeFlag",
        ito::ParamBase::Int,
        ...
        0,
        0,
        tr("Edge-Flag, currently not used").toLatin1().data());
    paramsOpt->append(param);
    param = ito::Param(
        "newValueInside",
        ito::ParamBase::Double,
        ...
        std::numeric_limits<double>::max(),
        1.0,
        tr("New value for pixels inside the geometric element").toLatin1().data());
    paramsOpt->append(param);
    param = ito::Param(
        "newValueOutside",
        ito::ParamBase::Double,
        ...
        std::numeric_limits<double>::max(),
        0.0,
        tr("New value for pixels outside the geometric element").toLatin1().data());
    paramsOpt->append(param);
...
//----------------------------------------------------------------------------------------------------------------------------------
template <typename _Tp>
void BasicFilters::fillGeoCircle(
    cv::Mat* dst,
    const ito::float64 x0,
    const ito::float64 y0,
    const ito::float64 radius,
    const bool inside,
    const bool outside,
    const _Tp insideVal,
    const _Tp outsideVal)
{
#if (USEOMP)
#pragma omp parallel num_threads(ito::AddInBase::getMaximumThreadCount())
    {
...
//----------------------------------------------------------------------------------------------------------------------------------
template <typename _Tp>
void BasicFilters::fillGeoEllipse(
    cv::Mat* dst,
    const ito::float64 x0,
    const ito::float64 y0,
    const ito::float64 radiusX,
    const ito::float64 radiusY,
    const bool inside,
    const bool outside,
    const _Tp insideVal,
    const _Tp outsideVal)
...
//----------------------------------------------------------------------------------------------------------------------------------
template <typename _Tp>
void BasicFilters::fillGeoRectangle(
    cv::Mat* dst,
    const ito::float64 x0,
    const ito::float64 y0,
    const ito::float64 x1,
    const ito::float64 y1,
    const bool inside,
    const bool outside,
    const _Tp insideVal,
    const _Tp outsideVal)
...
/*!\detail
   ...
   \date
*/
ito::RetVal BasicFilters::fillGeometricPrimitiv(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* paramsOpt,
    QVector<ito::ParamBase>* /*paramsOut*/)
{
    ...
    ito::DataObject* dObjPrimitive = (ito::DataObject*)(*paramsMand)[1].getVal<void*>();

    if (!dObjPrimitive)
    {
        return ito::RetVal(
            ito::retError, 0, tr("Error: geometricElement ptr empty").toLatin1().data());
    }
    ...
    {
        return ito::RetVal(
            ito::retError, 0, tr("Error: source is not a 2d matrix").toLatin1().data());
    }

    if (dObjPrimitive->getDims() != 2)
    {
        return ito::RetVal(
            ito::retError, 0, tr("Error: geometricElement is not a 2d matrix").toLatin1().data());
    }

    if (dObjPrimitive->getType() != ito::tFloat32 && dObjPrimitive->getType() != ito::tFloat64)
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("Error: geometricElement must be either float32 or float64").toLatin1().data());
    }
    ...
    // Type 2 Structure will be dataObject([8, 1], 'float32') with [[idx], [type], [x], [y], [0],
    // [0], [0],[0]]
    ...
    if (dObjPrimitive->getSize(0) == 1 && dObjPrimitive->getSize(1) > 8)
    {
        if (dObjPrimitive->getType() == ito::tFloat32)
            type = cv::saturate_cast<int>(dObjPrimitive->at<ito::float32>(0, 1));
        else
            type = cv::saturate_cast<int>(dObjPrimitive->at<ito::float64>(0, 1));
        ...
        default:
            return ito::RetVal(
                ito::retError,
                0,
                tr("Error: geometric primitive not supported for filling").toLatin1().data());
        case ito::Shape::Circle:
            // case ito::PrimitiveContainer::tCircle:
            if (dObjPrimitive->getType() == ito::tFloat32)
            {
                x0 = dObjPrimitive->at<ito::float32>(0, 2);
                y0 = dObjPrimitive->at<ito::float32>(0, 3);
                rA = dObjPrimitive->at<ito::float32>(0, 5);
                rB = dObjPrimitive->at<ito::float32>(0, 5);
            }
            else
            {
                x0 = dObjPrimitive->at<ito::float64>(0, 2);
                y0 = dObjPrimitive->at<ito::float64>(0, 3);
                rA = dObjPrimitive->at<ito::float64>(0, 5);
                rB = dObjPrimitive->at<ito::float64>(0, 5);
            }
            break;
        case ito::Shape::Ellipse:
            if (dObjPrimitive->getType() == ito::tFloat32)
            {
                ...
                y0 = dObjPrimitive->at<ito::float32>(0, 3);
                rA = dObjPrimitive->at<ito::float32>(0, 5);
                rB = dObjPrimitive->at<ito::float32>(0, 6);
            }
            else
            {
                x0 = dObjPrimitive->at<ito::float64>(0, 2);
                y0 = dObjPrimitive->at<ito::float64>(0, 3);
                rA = dObjPrimitive->at<ito::float64>(0, 5);
                rB = dObjPrimitive->at<ito::float64>(0, 6);
            }
            break;
        case ito::Shape::Rectangle:
            if (dObjPrimitive->getType() == ito::tFloat32)
            {
                x0 = dObjPrimitive->at<ito::float32>(0, 2);
                y0 = dObjPrimitive->at<ito::float32>(0, 3);
                x1 = dObjPrimitive->at<ito::float32>(0, 5);
                y1 = dObjPrimitive->at<ito::float32>(0, 6);
            }
            else
            {
                x0 = dObjPrimitive->at<ito::float64>(0, 2);
                y0 = dObjPrimitive->at<ito::float64>(0, 3);
                x1 = dObjPrimitive->at<ito::float64>(0, 5);
                y1 = dObjPrimitive->at<ito::float64>(0, 6);
            }
            break;
        case ito::Shape::Square:
            ...
            {
                x0 = dObjPrimitive->at<ito::float32>(0, 2) -
                    dObjPrimitive->at<ito::float32>(0, 5) / 2.0;
                y0 = dObjPrimitive->at<ito::float32>(0, 3) -
                    dObjPrimitive->at<ito::float32>(0, 5) / 2.0;
                x1 = dObjPrimitive->at<ito::float32>(0, 2) +
                    dObjPrimitive->at<ito::float32>(0, 5) / 2.0;
                y1 = dObjPrimitive->at<ito::float32>(0, 3) +
                    dObjPrimitive->at<ito::float32>(0, 5) / 2.0;
            }
            else
            {
                x0 = dObjPrimitive->at<ito::float64>(0, 2) -
                    dObjPrimitive->at<ito::float64>(0, 5) / 2.0;
                y0 = dObjPrimitive->at<ito::float64>(0, 3) -
                    dObjPrimitive->at<ito::float64>(0, 5) / 2.0;
                x1 = dObjPrimitive->at<ito::float64>(0, 2) +
                    dObjPrimitive->at<ito::float64>(0, 5) / 2.0;
                y1 = dObjPrimitive->at<ito::float64>(0, 3) +
                    dObjPrimitive->at<ito::float64>(0, 5) / 2.0;
            }
            break;
        }
    }
    else if (dObjPrimitive->getSize(0) > 6 && dObjPrimitive->getSize(1) == 1)
    {
        if (dObjPrimitive->getType() == ito::tFloat32)
            type = cv::saturate_cast<int>(dObjPrimitive->at<ito::float32>(1, 0));
        else
            type = cv::saturate_cast<int>(dObjPrimitive->at<ito::float64>(1, 0));
        ...
        case ito::Shape::Ellipse:
            if (dObjPrimitive->getType() == ito::tFloat32)
            {
                x0 = (dObjPrimitive->at<ito::float32>(2, 0) +
                      dObjPrimitive->at<ito::float32>(4, 0)) /
                    2.0;
                y0 = (dObjPrimitive->at<ito::float32>(3, 0) +
                      dObjPrimitive->at<ito::float32>(5, 0)) /
                    2.0;
                rA = fabs(
                         dObjPrimitive->at<ito::float32>(2, 0) -
                         dObjPrimitive->at<ito::float32>(4, 0)) /
                    2.0;
                rB = fabs(
                         dObjPrimitive->at<ito::float32>(3, 0) -
                         dObjPrimitive->at<ito::float32>(5, 0)) /
                    2.0;
            }
            else
            {
                x0 = (dObjPrimitive->at<ito::float64>(2, 0) +
                      dObjPrimitive->at<ito::float64>(4, 0)) /
                    2.0;
                y0 = (dObjPrimitive->at<ito::float64>(3, 0) +
                      dObjPrimitive->at<ito::float64>(5, 0)) /
                    2.0;
                rA = fabs(
                         dObjPrimitive->at<ito::float64>(2, 0) -
                         dObjPrimitive->at<ito::float64>(4, 0)) /
                    2.0;
                rB = fabs(
                         dObjPrimitive->at<ito::float64>(3, 0) -
                         dObjPrimitive->at<ito::float64>(5, 0)) /
                    2.0;
            }
            break;
        ...
        case ito::Shape::Square:
            if (dObjPrimitive->getType() == ito::tFloat32)
            {
                x0 = dObjPrimitive->at<ito::float32>(2, 0);
                y0 = dObjPrimitive->at<ito::float32>(3, 0);
                x1 = dObjPrimitive->at<ito::float32>(4, 0);
                y1 = dObjPrimitive->at<ito::float32>(5, 0);
            }
            else
            {
                x0 = dObjPrimitive->at<ito::float64>(2, 0);
                y0 = dObjPrimitive->at<ito::float64>(3, 0);
                x1 = dObjPrimitive->at<ito::float64>(4, 0);
                y1 = dObjPrimitive->at<ito::float64>(5, 0);
            }
            break;
        }
    }
    else
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("Error: geometricElement must be either marker-style (8x1) or primitive-style "
               "(1x11)")
                .toLatin1()
                .data());
    }
    ...
    case ito::Shape::Ellipse:
        ...
        y1 = 0.0;
        x1 = 0.0;
        if (fabs(rA - rB) < dObjDst->getAxisScale(xDim) &&
            fabs(rA - rB) < dObjDst->getAxisScale(yDim))
        ...
        if (!ito::isNotZero(rA) || !ito::isNotZero(rB))
        {
            return ito::RetVal(
                ito::retError,
                0,
                tr("Error: radii of geometricElement must not be zero").toLatin1().data());
        }
        break;
    case ito::Shape::Rectangle:
    case ito::Shape::Square:
    ...
    if (!ito::isFinite(rA) || !ito::isFinite(rB) || !ito::isFinite(x0) || !ito::isFinite(y1) ||
        !ito::isFinite(x1) || !ito::isFinite(y0))
    {
        return ito::RetVal(
            ito::retError,
            0,
            tr("Error: coordinates of geometricElement must be finite").toLatin1().data());
    }
    ...
    case ito::tFloat64: {
        ...
        default:
            return ito::RetVal(
                ito::retError,
                0,
                tr("Error: geometric primitivee not supported for filling").toLatin1().data());
        }
        break;
    }
    default:
        return ito::RetVal(
            ito::retError,
            0,
            tr("Error: destination object type is not suppirted for filling").toLatin1().data());
    }
...
//----------------------------------------------------------------------------------------------------------------------------------
const QString BasicFilters::calcRadialMeanFilterDoc = QObject::tr(
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::calcRadialMeanFilterParams(
    QVector<ito::Param>* paramsMand, QVector<ito::Param>* paramsOpt, QVector<ito::Param>* paramsOut)
{
    ito::RetVal retval = prepareParamVectors(paramsMand, paramsOpt, paramsOut);
    if (!retval.containsError())
    {
        ...
        param = ito::Param(
            ...
            ito::ParamBase::DObjPtr | ito::ParamBase::In | ito::ParamBase::Out,
            NULL,
            tr("Output image (1xN) of the same type than the input image, where N corresponds to "
               "different radiuses.")
                .toLatin1()
                .data());
        paramsMand->append(param);
        param = ito::Param(
            ...
            std::numeric_limits<double>::max(),
            0.01,
            tr("step size of the radius for the discretization").toLatin1().data());
        paramsMand->append(param);
        param = ito::Param(
            "centerX",
            ito::ParamBase::Double | ito::ParamBase::In,
            ...
            std::numeric_limits<double>::max(),
            0.0,
            tr("x-coordinate of radial center").toLatin1().data());
        paramsOpt->append(param);
        param = ito::Param(
            "centerY",
            ito::ParamBase::Double | ito::ParamBase::In,
            ...
            std::numeric_limits<double>::max(),
            0.0,
            tr("y-coordinate of radial center").toLatin1().data());
        paramsOpt->append(param);
    }
...
void calcRadialMeanHelper(
    const ito::DataObject& input,
    cv::Mat* bucket,
    double& cx,
    double& cy,
    double& rStep,
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal BasicFilters::calcRadialMeanFilter(
    QVector<ito::ParamBase>* paramsMand,
    QVector<ito::ParamBase>* paramsOpt,
    QVector<ito::ParamBase>* /*paramsOut*/)
{
    ...
    const ito::DataObject input = ito::dObjHelper::squeezeConvertCheck2DDataObject(
    ...
    if (!retval.containsError())
    {
        ...
        if (nrBuckets == 0)
        {
            retval += ito::RetVal(
                ito::retError,
                0,
                "radiusStep is too big. The maximum available radius value is much smaller");
        }
    }
    ...
    if (!retval.containsError())
    {
        ...
        case ito::tUInt8:
            ...
            calcRadialMeanHelper<ito::uint8, ito::int32>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
            retval += vals.convertTo(*output, ito::tUInt8);
            break;
        case ito::tUInt16:
            vals.zeros(nrBuckets, ito::tInt32);
            calcRadialMeanHelper<ito::uint16, ito::int32>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
        ...
        case ito::tUInt32:
            vals.zeros(nrBuckets, ito::tInt32);
            calcRadialMeanHelper<ito::uint32, ito::int32>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
        ...
        case ito::tInt8:
            vals.zeros(nrBuckets, ito::tInt32);
            calcRadialMeanHelper<ito::int8, ito::int32>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
        ...
        case ito::tInt16:
            vals.zeros(nrBuckets, ito::tInt32);
            calcRadialMeanHelper<ito::int16, ito::int32>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
        ...
        case ito::tInt32:
            vals.zeros(nrBuckets, ito::tInt32);
            calcRadialMeanHelper<ito::int32, ito::int32>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
            *output = vals;
            break;
        case ito::tFloat32:
            ...
            calcRadialMeanHelper<ito::float32, ito::float64>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
        ...
        case ito::tFloat64:
            vals.zeros(nrBuckets, ito::tFloat64);
            calcRadialMeanHelper<ito::float64, ito::float64>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
            *output = vals;
            break;
        case ito::tComplex64:
            ...
            calcRadialMeanHelper<ito::complex64, ito::complex128>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
        ...
        case ito::tComplex128:
            vals.zeros(nrBuckets, ito::tComplex128);
            calcRadialMeanHelper<ito::complex128, ito::complex128>(
                input, vals.getCvPlaneMat(0), cX, cY, rStep, usedBuckets);
            vals = vals.at(ito::Range::all(), ito::Range(0, usedBuckets));
            *output = vals;
            break;
