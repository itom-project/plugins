...
//    options.c_cc[VTIME] = 0;    // Set timeout
    ...
    {
        return ito::RetVal(ito::retError, 0, QObject::tr("error setting parameters").toLatin1().data());   // Ea=dataIO("ROS4Bridge",0,9600,"\r")rror while writing
    }
...
//----------------------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4BridgeInterface::getAddInInst(ito::AddInBase **addInInst)
{
    NEW_PLUGININSTANCE(ROS4Bridge)
    return ito::retOk;
}

//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4BridgeInterface::closeThisInst(ito::AddInBase **addInInst)
{
   REMOVE_PLUGININSTANCE(ROS4Bridge)
   return ito::retOk;
}

//----------------------------------------------------------------------------------------------------------------------------------
ROS4BridgeInterface::ROS4BridgeInterface()
{
    m_type = ito::typeDataIO | ito::typeRawIO;
    setObjectName("ROS4Bridge");

    m_description = tr("itom-plugin for a serial port communication");

    //for the docstring, please don't set any spaces at the beginning of the line.
/*    char docstring[] = \
"ROS4Bridge is a itom-Plugin which gives direct access to serial ports.\nIt is used by different plugins for communication, (e.g. 'PIPiezoCtrl', 'UhlActuator', 'LeicaMotorFocus').\n\
...
.. \n\
    \n\
    s = dataIO(\"ROS4Bridge\",port=1,baud=9600,endline=\"\",bits=8,stopbits=1,parity=0,flow=16) \n\
    \n\
    #send command \n\
    ...
    m_detaildescription = tr("ROS4Bridge is a itom-Plugin which gives direct access to serial ports.\nIt is used by different plugins for communication, (e.g. 'PIPiezoCtrl', 'UhlActuator', 'LeicaMotorFocus').\n\
The plugin is implemented for Windows or Linux; the possible baudrates depend on the possibilities of the operating system. \n\
\n\
flow bitmask \n\
-------------- \n\
...
//----------------------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------------------
const ito::RetVal ROS4Bridge::showConfDialog(void)
{
    if (qobject_cast<QApplication*>(QCoreApplication::instance()))
        return apiShowConfigurationDialog(this, new dialogROS4Bridge(this, (void*)this, m_identifier, (int)(sizeof(SerialPort::baudRates) / sizeof(int))));
    else
        return ito::retOk;
/*    dialogROS4Bridge *confDialog = new dialogROS4Bridge((void*)this, m_identifier);
...
//----------------------------------------------------------------------------------------------------------------------------------
ROS4Bridge::ROS4Bridge() : AddInDataIO(), m_debugMode(false), m_debugIgnoreEmpty(false)
{
    m_preBuf.clear();

    ito::Param paramVal("name", ito::ParamBase::String | ito::ParamBase::Readonly | ito::ParamBase::NoAutosave, "ROS4Bridge", NULL);
...
/*    //now create dock widget for this plugin
    DockWidgetROS4Bridge *ROS4BridgeWidget = new DockWidgetROS4Bridge(m_params, m_uniqueID);
    connect(this, SIGNAL(parametersChanged(QMap<QString, ito::Param>)), ROS4BridgeWidget, SLOT(valuesChanged(QMap<QString, ito::Param>)));
    connect(this, SIGNAL(uniqueIDChanged(const int)), ROS4BridgeWidget, SLOT(uniqueIDChanged(const int)));
    connect(this, SIGNAL(serialLog(QByteArray, QByteArray, const char)), ROS4BridgeWidget, SLOT(serialLog(QByteArray, QByteArray, const char)));*/
    ...
    if (hasGuiSupport())
    {
        DockWidgetROS4Bridge *dw = new DockWidgetROS4Bridge(this);
...
*/
ito::RetVal ROS4Bridge::getParam(QSharedPointer<ito::Param> val, ItomSharedSemaphore *waitCond)
{
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4Bridge::setParam(QSharedPointer<ito::ParamBase> val, ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4Bridge::init(QVector<ito::ParamBase> *paramsMand, QVector<ito::ParamBase> *paramsOpt, ItomSharedSemaphore *waitCond)
{
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4Bridge::close(ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retval;
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4Bridge::startDevice(ItomSharedSemaphore *waitCond)
{
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4Bridge::stopDevice(ItomSharedSemaphore *waitCond)
{
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4Bridge::acquire(const int /*trigger*/, ItomSharedSemaphore *waitCond)
{
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4Bridge::getVal(QSharedPointer<char> data, QSharedPointer<int> length, ItomSharedSemaphore *waitCond)
{
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4Bridge::setVal(const char *data, const int datalength, ItomSharedSemaphore *waitCond)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    const char *buf = data;
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal ROS4Bridge::execFunc(const QString funcName, QSharedPointer<QVector<ito::ParamBase> > paramsMand, QSharedPointer<QVector<ito::ParamBase> > paramsOpt, QSharedPointer<QVector<ito::ParamBase> > paramsOut, ItomSharedSemaphore *waitCond /*= NULL*/)
{
    ItomSharedSemaphoreLocker locker(waitCond);
    ito::RetVal retval;
...
//----------------------------------------------------------------------------------------------------------------------------------
void ROS4Bridge::dockWidgetVisibilityChanged(bool visible)
{
    if (qobject_cast<QApplication*>(QCoreApplication::instance()) && getDockWidget())
    {
        DockWidgetROS4Bridge *dw = qobject_cast<DockWidgetROS4Bridge*>(getDockWidget()->widget());
        if (visible)
        {
