    ...
    m_colouredOutput(false)
{
    ...
    //DockWidgetIDSuEye *dw = new DockWidgetIDSuEye(this);
...
ito::RetVal IDSuEye::init(QVector<ito::ParamBase> *paramsMand, QVector<ito::ParamBase> *paramsOpt, ItomSharedSemaphore *waitCond)
{
    ...
    if (!retVal.containsError())
    {
        ...
        if (!retVal.containsError())
        {
            ...
            {
                retVal += checkError(is_SetColorMode(m_camera, IS_CM_MONO8));
            }
...
ito::RetVal IDSuEye::close(ItomSharedSemaphore *waitCond)
{
    ...
    //if old memory exists try to free it
    if (m_pMemory)
    {
        is_FreeImageMem(m_camera, m_pMemory->ppcImgMem, m_pMemory->pid);
        delete m_pMemory;
        m_pMemory = NULL;
    }
...
ito::RetVal IDSuEye::setParam(QSharedPointer<ito::ParamBase> val, ItomSharedSemaphore *waitCond)
{
    ...
    if(!retValue.containsError())
    {
        ...
        {
            retValue += ito::RetVal(ito::retError, 0, "not yet implemented");
        }
        ...
        {
            retValue += checkError(is_SetHardwareGain(m_camera, qRound(val->getVal<double>()), IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER));
            if (!retValue.containsError())
            {
                retValue += synchronizeCameraSettings(sMasterGain);
            }
        }
        else if (key == "offset")
        {
...
ito::RetVal IDSuEye::acquire(const int trigger, ItomSharedSemaphore *waitCond)
{
    ...
    {
        //get timeout in 10ms steps, minimum is 40ms
        int timeout = std::max(4,qRound(m_params["timeout"].getVal<double>() * 100.0)); //*100 gets from seconds to 10ms base
        ...
        INT code = is_FreezeVideo(m_camera, timeout);
        if (code == IS_TRANSFER_ERROR)
        {
            m_acquisitionRetVal = ito::RetVal(ito::retError, 0, "failed capturing an image, transfer failed. Check / reduce pixel clock");
        }
        else if (code != IS_SUCCESS)
        {
            m_acquisitionRetVal = checkError(code);
        }
        else
        {
            m_pMemory->imageAvailable = true;
        }
    }
...
ito::RetVal IDSuEye::retrieveData(ito::DataObject *externalDataObject)
{
    ...
    if (!m_pMemory || m_pMemory->imageAvailable == false)
    {
    ...
    if (!retValue.containsError())
    {
        ...
        is_InquireImageMem(m_camera, m_pMemory->ppcImgMem, m_pMemory->pid, &pnX, &pnY, &pnBits, &pnPitch);
        int bytesPerLine = m_pMemory->width * m_pMemory->bitspixel / 8;
        char *startPtr = m_pMemory->ppcImgMem;
        // pnPitch is in bits

        //in rgba8_packed, the alpha channel is 0, this must be 255 in itom (opaque)
        if (m_colouredOutput)
        {
            UCHAR *sourcePtr = (UCHAR*)(m_pMemory->ppcImgMem + 3);
...
ito::RetVal IDSuEye::synchronizeCameraSettings(int what /*= sAll*/)
{
    ...
    if (what & sRoi)
    {
        ...
        if (!rettemp.containsError())
        {
            ...
            it->setMeta(new ito::IntMeta(offsetMin.s32X, offsetMax.s32X, offsetInc.s32X), true);
            it->setFlags(0);
            it = m_params.find("y0");
            it->setVal<int>(offset.s32Y);
            it->setMeta(new ito::IntMeta(offsetMin.s32Y, offsetMax.s32Y, offsetInc.s32Y), true);
            it->setFlags(0);
            it = m_params.find("x1");
            it->setVal<int>(size.s32X + offset.s32X - 1);
            it->setMeta(new ito::IntMeta(offset.s32X + sizeMin.s32X - 1, offset.s32X + sizeMax.s32X - 1, sizeInc.s32X), true);
            it->setFlags(0);
            it = m_params.find("y1");
            it->setVal<int>(size.s32Y + offset.s32Y - 1);
    ...
    if (what & sMasterGain)
    {
        ...
        it->setVal<int>(is_SetHardwareGain(m_camera, IS_GET_MASTER_GAIN, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER));
    }
    ...
    if (what & sOffset)
    {
        ...
        UINT val, val2;
        ito::RetVal retTemp = checkError(is_Blacklevel(m_camera, IS_BLACKLEVEL_CMD_GET_CAPS, (void*)&val, sizeof(val)));

        if (!retTemp.containsError())
        {
            it->setFlags((val & IS_BLACKLEVEL_CAP_SET_OFFSET) ? 0 : ito::ParamBase::Readonly);
    ...
    if (what & sBppAndColorMode)
    {
        ...
        rettemp = checkError(is_GetColorDepth(m_camera, &bpp, &colmode));
...
ito::RetVal IDSuEye::checkData(ito::DataObject *externalDataObject)
{
    ...
    if (!m_pMemory || (m_pMemory->height != futureHeight || m_pMemory->width != futureWidth || m_pMemory->bitspixel != m_bitspixel))
    {
        //if old memory exists try to free it
        if (m_pMemory)
        {
        ...
        m_pMemory = new MemoryStruct();
        m_pMemory->height = futureHeight;
        m_pMemory->width = futureWidth;
        m_pMemory->bitspixel = m_bitspixel;
        m_pMemory->pid = 0;
        m_pMemory->ppcImgMem = NULL;
        m_pMemory->imageAvailable = false;

        ito::RetVal retval = checkError(is_AllocImageMem(m_camera, futureWidth, futureHeight, m_bitspixel, &(m_pMemory->ppcImgMem), &(m_pMemory->pid)));
        if (retval.containsError())
        {
            delete m_pMemory;
            m_pMemory = NULL;
        ...
        {
            retval += checkError(is_SetImageMem(m_camera, m_pMemory->ppcImgMem, m_pMemory->pid));
            if (retval.containsError())
            {
                is_FreeImageMem(m_camera, m_pMemory->ppcImgMem, m_pMemory->pid);
                delete m_pMemory;
